...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, tar_layer=None):
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, tmp_score=None):
        self_output, att_score = self.self(input_tensor, attention_mask, tmp_score)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, tmp_score=None):
        attention_output, att_score = self.attention(hidden_states, attention_mask, tmp_score)
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, tar_layer=None, tmp_score=None):
        all_encoder_layers = []
        att_score = None
...
class BertForPreTrainingLossMask(PreTrainedBertModel):
    ...
    def __init__(self, config, num_labels=2, num_rel=0):
        ...
        self.crit_next_sent = nn.CrossEntropyLoss()
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
        ...
        next_sentence_loss = self.crit_next_sent(
            seq_relationship_score.view(-1, self.num_labels), next_sentence_label.view(-1))
        if pair_x is None or pair_y is None or pair_r is None or pair_pos_neg_mask is None or pair_loss_mask is None:
            return masked_lm_loss, next_sentence_loss

        # pair and relation
        pair_x_output_masked = gather_seq_out_by_pos(sequence_output, pair_x)
        pair_y_output_masked = gather_seq_out_by_pos(sequence_output, pair_y)
        pair_loss = self.crit_pair_rel(
            pair_x_output_masked, pair_y_output_masked, pair_r, pair_pos_neg_mask)
        pair_loss = loss_mask_and_normalize(pair_loss, pair_loss_mask)
